<?php
	...
	if(isset($_SESSION['id']) != ""){
		...
		if( ( ($localName != $isItOneUser['pseudo'] || $isUser) AND $getLocation[4] == "profil.php" AND $localName != null AND $_SESSION['id'] == $localId AND $isUser) 
			||  ($localName != $isItOneUser['pseudo'] AND $getLocation[4] == "profil.php" AND $localName != null AND !$isUser)){
			...
			if(isset($_POST['formNewPwd'])) {
				...
				if($localUser["motdepasse"]==sha1($_POST["oldPwd"])){
					...
					if (sha1($_POST["newPwd"]) !== $localUser["motdepasse"]) {
						...
						$erreur = "Mot de passe changé avec succès";
					}
					else{
...
?>
		<div class="w-100" style="margin-top: 100px !important;">
			...
			<div class="align-items-center bg-light card d-flex justify-content-center m-4 p-4 " data-animate-top>
				...
				<img id="profilId" style="height:300px;" src="<?php echo $localPdp; ?>" alt="" class="img-fluid">
				...
				<h1><?php if($localType == "user"){echo $localName.' ('.$localMail.')';}else{echo $localName." [ADMIN]";}?></h1>
				
				<br/>
				
				<ul class="font-weight-bold list-unstyled text-center">
					<li class="editTxt"><i class="fa fa-edit"></i><a id="edit-pic">Modifier ma photo profil</a></li>
			...
			<div class="align-items-center bg-light card d-flex justify-content-center m-4 p-4 " data-animate-top>
				<a href="portfolio.php" class="text-center text-dark text-decoration-none">
					...
					<h6>
						Grâce à mon portfolio, j'accède à ma galerie d'image personnelle et je partage mes créations et mes clichés favoris avec l'ensemble de la communauté YotaFootball !
					</h6>
					<div style="width: 90% !important; display:inline-block; cursor:pointer;" >
					<?php
		...
		<script>
			...
			document.getElementById('edit-closePic').onclick = function() {
				...
				document.getElementById('profilId').src = <?php echo '"'.$localPdp.'"';?>;
